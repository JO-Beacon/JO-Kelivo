...
class WorkspaceDirectoryAccess(private val context: Context) {
    ...
    fun requestStorageAccess(result: MethodChannel.Result) {
        val activity = attachedActivity ?: run {
            result.error("foreground_activity_required", "Open JO-AIClient to grant storage access.", null)
            return
        }
        if (hasStorageAccess()) {
            result.success(true)
    ...
    fun pick(result: MethodChannel.Result) {
        val activity = attachedActivity ?: run {
            result.error("foreground_activity_required", "Open JO-AIClient to choose a folder.", null)
            return
        }
        if (!hasStorageAccess()) {
