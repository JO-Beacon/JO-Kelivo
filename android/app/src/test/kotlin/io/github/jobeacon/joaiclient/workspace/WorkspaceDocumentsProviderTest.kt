package io.github.jobeacon.joaiclient.workspace
...
import io.github.jobeacon.joaiclient.IncomingShareHandler
...
class WorkspaceDocumentsProviderTest {
    ...
    fun exposesNamedWorkspacesWithoutFlutterOrDuplicateDatabase() {
        ...
        assertEquals(listOf("JO-AIClient"), provider.queryRoots(null).strings(Root.COLUMN_TITLE))
    ...
    fun honorsRequestedProjectionsAndReadOnlyFlags() {
        ...
        provider.queryRoots(arrayOf(Root.COLUMN_TITLE)).use {
            ...
            assertEquals("JO-AIClient", it.getString(0))
        }
        children().use {
            assertTrue(it.moveToFirst())
