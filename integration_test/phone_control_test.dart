...
// Run on an unlocked Android test device with JO-AIClient's accessibility service
...
void main() {
  ...
  setUpAll(() async {
    ...
    expect(
      ...
      isTrue,
      reason:
          'Enable JO-AIClient phone control in Android Accessibility settings on this test device.',
    );
    await Future<void>.delayed(const Duration(milliseconds: 500));
  });
  ...
  testWidgets('phone control reads and operates real Android windows', (
    ...
    expect(snapshot['package_name'], 'io.github.jobeacon.joaiclient');
    ...
    await call({
      'action': 'open_app',
      'package_name': 'io.github.jobeacon.joaiclient',
    });
    snapshot = await waitForPackage(
      (name) => name == 'io.github.jobeacon.joaiclient',
    );
    expect(snapshot['package_name'], 'io.github.jobeacon.joaiclient');
  });
}
