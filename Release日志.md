...
当前代码版本：`0.1.16+16`（已合入 main，待发布；发布正文已更新为 1.3.0 口径）。当前实现基座：Kelivo `1.3.0+79`（上游 `master` commit `5abbcf58`，2026-09-17；上游未打 `v1.3.0` 标签，按提交集合取用；关于页声明值同为 `1.3.0 / 79`）。GitHub 上最新已公开 Release 为 `0.1.15+15`（2026-09-12 发布）。`0.1.16` 起新增 Windows ARM64 原生安装包。
本地技术收口状态（2026-09-21，未发布）：全量测试 5416 通过／41 跳过／0 失败，全仓库分析无问题；Android 三个 ABI 正式版 APK、Windows x64 便携包和安装包均已本地重建。Android 真机、真实字体联网下载和真实使用场景人工验收仍未执行。
