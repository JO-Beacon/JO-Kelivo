...
Future<SettingsProvider> _pumpPane(
  ...
  // The backup pane can keep an indeterminate progress indicator alive while
  // an optional provider is still loading. Layout assertions only need a
  // stable frame, so do not wait for every animation to finish.
  await tester.pump();
      ...
      (tester) async {
        ...
        for (final title in [
          ...
          expect(
            ...
            findsOneWidget,
            reason: 'missing backup section: $title',
          );
          ...
          if (title == l10n.backupPageLocalBackup) {
            final card = find
                ...
                .first;
            final export = find.text(l10n.backupPageLocalBackupAction);
            final import = find.descendant(
              of: card,
              matching: find.text(l10n.backupPageRestore),
            );
