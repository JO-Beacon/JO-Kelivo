...
void main() {
  testWidgets('已有会话过期错误码仍显示本地化说明', (tester) async {
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: Scaffold(
          body: McpErrorDetailsSheet(
            serverName: 'server',
            message: 'mcp_session_stream_expired',
            onReconnect: () {},
          ),
        ),
      ),
    );
    final context = tester.element(find.byType(McpErrorDetailsSheet));
    expect(
      find.text(AppLocalizations.of(context)!.mcpPageSessionExpiredDetails),
      findsOneWidget,
    );
    expect(find.text('mcp_session_stream_expired'), findsNothing);
  });
