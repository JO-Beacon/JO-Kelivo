...
import 'package:Kelivo/desktop/widgets/desktop_dialog_style.dart';
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final provider = await _openEditor(
        ...
        args,
        desktop: true,
        environment: {'FLAGS': '  literal value  '},
      );
      final dialogContext = tester.element(find.byType(Dialog));
      expect(
        (tester.widget<Dialog>(find.byType(Dialog)).child! as ConstrainedBox)
            .constraints,
        DesktopDialogStyle.formConstraints(dialogContext),
      );
      await tester.enterText(
        _fieldWithText('Imported server'),
        'Renamed desktop',
      );
      await tester.tap(find.text('Save'));
      await tester.pumpAndSettle();
      expect(provider.getById('guest')!.args, args);
      expect(provider.getById('guest')!.name, 'Renamed desktop');
      expect(provider.getById('guest')!.env, {'FLAGS': '  literal value  '});
