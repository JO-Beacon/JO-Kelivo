...
void main() {
  ...
  test('migrates chat data across multiple message batches', () async {
    ...
    final migratedMessages = await chatService.loadMessages(conversation.id);
    // The normal chat surface follows the selected active tree path. The
    // unselected sibling remains persisted and is checked separately below.
    expect(migratedMessages, hasLength(messageCount - 1));
    expect(migratedMessages.first.id, 'message-0');
    expect(migratedMessages[126].id, 'message-127');
    ...
    expect(
      await chatService.loadPersistedMessageIds(conversation.id),
  ...
  test('repairs duplicate versions and preserves selection identity', () async {
    ...
    final migrated = await chatService.loadMessages(conversation.id);
    expect(migrated.map((m) => m.id).toList(), ['dup-b', 'empty-b']);
    expect(
      await chatService.loadPersistedMessageIds(conversation.id),
      containsAll(<String>['dup-a', 'dup-b', 'empty-a', 'empty-b']),
    );
    final duplicateGroup = await chatService.loadMessagesForGroups(
      conversation.id,
      const ['dup-group', ''],
    );
    expect(
