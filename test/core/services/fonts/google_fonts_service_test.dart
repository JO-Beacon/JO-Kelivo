...
void main() {
  ...
  test('过期目录联网失败必须报错，不能静默使用旧目录', () async {
    ...
    await expectLater(service.loadCatalog(), throwsA(isA<HttpException>()));
    await expectLater(
  ...
  test('目录缓存写入失败明确报告且清理临时文件', () async {
    await Directory(p.join(dir.path, 'catalog.json')).create();
    final service = GoogleFontsService(
      cacheDirectory: dir,
      client: MockClient(
        (_) async => http.Response(catalog([entry('Abel')]), 200),
