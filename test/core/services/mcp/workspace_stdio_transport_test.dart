...
void main() {
  test('停止沙盒进程失败时保留诊断，不报告正常关闭', () async {
    ...
    final closing = expectLater(transport.onClose, throwsA(isA<StateError>()));
    transport.close();
    await closing;
    expect(transport.failed, isTrue);
    expect(transport.describeError('closed'), contains('cancel failed'));
    transport.close();
  });
