...
void main() {
  group('NotificationService.shouldShowChatCompleted', () {
    test('suppresses only the visible current Android conversation', () {
      expect(
    ...
    test('requires Android and notification mode', () {
      expect(
        NotificationService.shouldShowChatCompleted(
          isAndroid: false,
          notifyModeEnabled: true,
  ...
  test('payload accepts only non-empty completion targets', () {
    expect(
  ...
  test(
    'startup registers notification handling before reading the mode',
    () async {
