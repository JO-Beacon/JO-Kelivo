...
void main() {
  ...
  group('FileLinkResolver.resolveToHostFile', () {
    ...
    test('resolves a workspace file under the host root', () async {
      ...
      expect(p.equals(resolved!.path, file.path), isTrue);
      expect(await resolved.readAsBytes(), await file.readAsBytes());
    });
    ...
    test(
      ...
      () async {
        ...
        expect(attachResolved?.path, isNotNull);
        expect(p.equals(attachResolved!.path, attach.path), isTrue);
        expect(await attachResolved.readAsBytes(), await attach.readAsBytes());
        expect(outputResolved?.path, isNotNull);
        expect(p.equals(outputResolved!.path, output.path), isTrue);
        expect(await outputResolved.readAsBytes(), await output.readAsBytes());
      },
    );
    ...
    test('resolves skill files and rejects skill escapes', () async {
      ...
      expect(resolved?.path, isNotNull);
      expect(p.equals(resolved!.path, skillFile.path), isTrue);
      expect(await resolved.readAsBytes(), await skillFile.readAsBytes());
    ...
    test('resolves encoded chat output with Chinese name', () async {
      ...
      expect(resolved?.path, isNotNull);
      expect(p.equals(resolved!.path, output.path), isTrue);
      expect(await resolved.readAsBytes(), await output.readAsBytes());
    });
    ...
    test(
      ...
      () async {
        ...
        expect(directory?.path, isNotNull);
        expect(p.equals(directory!.path, workspaceRoot.path), isTrue);
        expect(
      ...
      () async {
        ...
        try {
          ...
          expect(
            ...
            predicate<String>((path) => p.equals(path, file.path)),
          );
