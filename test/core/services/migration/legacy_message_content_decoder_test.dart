...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(imagePart.uri, SandboxPathResolver.canonicalize(image.path));
      ...
      expect(filePart.uri, SandboxPathResolver.canonicalize(pdf.path));
  ...
  test('missing local file still creates unavailable part', () async {
    ...
    final image = result.parts.single as ImagePart;
    expect(image.uri, SandboxPathResolver.canonicalize(missing));
