...
class _FakePathProviderPlatform extends PathProviderPlatform {
  ...
  final String path;
  Completer<void>? supportPathRequested;
  Completer<void>? supportPathGate;

  Future<String?> _getPath() async {
    supportPathRequested?.complete();
    await supportPathGate?.future;
    return path;
  }
...
void main() {
  ...
  test(
    'disabling waits for queued writes and releases the active file',
    () async {
      final pathRequested = Completer<void>();
      final pathGate = Completer<void>();
      fakePathProvider
        ..supportPathRequested = pathRequested
        ..supportPathGate = pathGate;
      await RequestLogger.setEnabled(true);
      RequestLogger.logLine('queued request');
      await pathRequested.future;

      final disabled = RequestLogger.setEnabled(false);
      pathGate.complete();
      await disabled;

      final active = File('${logsDir.path}/logs.txt');
      expect(await active.readAsString(), contains('queued request'));
      await active.delete();
