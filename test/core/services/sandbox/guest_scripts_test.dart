...
void main() {
  ...
  test(
    ...
    () async {
      ...
      await _runSh(
        GuestScripts.writeFile(
          path: './repositories',
          body: 'https://official.test/\n',
        ),
        dir,
      );
...
Future<void> _runSh(String script, Directory directory) async {
  var shell = '/bin/sh';
  if (Platform.isWindows) {
    // Git for Windows 自带 POSIX shell；所有读写均限制在测试临时目录内。
    final git = await Process.run('git', ['--exec-path']);
    expect(git.exitCode, 0, reason: '查找 Git shell 失败：${git.stderr}');
    final root = Directory((git.stdout as String).trim()).parent.parent.parent;
    shell = '${root.path}/bin/sh.exe';
    expect(
      File(shell).existsSync(),
      isTrue,
      reason: '缺少 Git for Windows shell：$shell',
    );
  }
