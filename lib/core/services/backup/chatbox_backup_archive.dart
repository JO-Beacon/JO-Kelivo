...
class ChatboxArchiveException implements Exception {
  final String message;
  const ChatboxArchiveException(this.message);
  @override
...
class ChatboxBackupArchive {
  ...
  static Future<ChatboxBackupReadResult> readZipV2({
    ...
    try {
      try {
        ...
        if (e is ChatboxArchiveException) rethrow;
        throw ChatboxArchiveException('Unable to read Chatbox backup ZIP: $e');
      }
  ...
  static Future<ChatboxBackupReadResult> _interpretArchive(
    ...
    for (final entry in archive) {
      ...
      if (entry.isSymbolicLink) {
        throw ChatboxArchiveException('Unsafe ZIP entry path: ${entry.name}');
      }
      final path = _normalizedArchivePath(entry.name);
      if (entries.containsKey(path)) {
        throw ChatboxArchiveException('Duplicate ZIP entry: $path');
      }
      fileCount++;
      if (fileCount > maxFileEntries) {
        throw const ChatboxArchiveException(
    ...
    for (final raw in sessions) {
      ...
      if (!_isBackupSession(parsed)) {
        throw ChatboxArchiveException('Invalid session entry: $sessionPath');
      }
      if ((parsed['id'] ?? '').toString() != sessionId) {
        throw ChatboxArchiveException(
  ...
  static Future<void> _publishStagedFile(File staged, File dest) async {
    ...
    try {
      ...
      if (e is ChatboxArchiveException) rethrow;
      throw ChatboxArchiveException('Unable to publish backup resource: $e');
    }
  }
  ...
  static Map<String, dynamic> _parseManifest(Uint8List bytes) {
    ...
    if (sessions.length > maxSessions) {
      throw const ChatboxArchiveException('Backup contains too many sessions.');
    }
    ...
    if (resources.length > maxResources) {
      throw const ChatboxArchiveException(
        'Backup contains too many resources.',
      );
    }
  ...
  static void _validateManifestMembership(
    ...
    void addDescriptor(Object? raw, {required bool requiredId}) {
      ...
      if (!entries.containsKey(path)) {
        throw ChatboxArchiveException('Backup entry is missing: $path');
      }
      if (requiredId && (descriptor['id'] ?? '').toString().isEmpty) {
        throw ChatboxArchiveException('Backup entry is missing an id: $path');
      }
    }
  ...
  static void _assertEntryBudget({
    ...
    if (uncompressed > limit) {
      throw ChatboxArchiveException('Backup entry is too large: $path');
    }
  ...
  static _ExtractedEntry _extractVerified(
    ...
    if (expectedSize is int && entry.size != expectedSize) {
      throw ChatboxArchiveException('Backup entry size mismatch: $path');
    }
    final limit = _isBackupJsonPath(path)
        ? maxJsonEntryBytes
        : maxResourceEntryBytes;
    ...
    try {
      ...
      if (verifyChecksum) {
        ...
        if (checksumRaw is! Map) {
          throw ChatboxArchiveException(
            'Backup entry checksum mismatch: $path',
          );
        }
      ...
      sink.abort();
      if (e is ChatboxArchiveException) rethrow;
      throw ChatboxArchiveException('Unable to read backup entry: $path');
    } finally {
      entry.clear();
    }
  }
  ...
  static void _validateManifestGraph(Map<String, dynamic> manifest) {
    ...
    for (final raw in sessions) {
      ...
      if (!sessionIds.add(id)) {
        throw ChatboxArchiveException('Duplicate session id in manifest: $id');
      }
      final meta = session['meta'];
      if (meta is Map && (meta['id'] ?? '').toString() != id) {
        throw ChatboxArchiveException(
          'Session metadata id does not match: $id',
        );
      }
    }
    ...
    for (final raw in manifest['resources'] as List) {
      ...
      if (!resourceIds.add(id)) {
        throw ChatboxArchiveException('Duplicate resource id in manifest: $id');
      }
  ...
  static void _requireJsonDescriptor(Object raw, String label) {
    if (raw is! Map) {
      throw ChatboxArchiveException(
        'Invalid Chatbox backup manifest ($label).',
      );
    }
  ...
  static String _normalizedArchivePath(String raw) {
    if (raw.contains('\u0000')) {
      throw ChatboxArchiveException('Unsafe ZIP entry path: $raw');
    }
  ...
  static void _assertSafeArchivePath(String path) {
    if (path.isEmpty) {
      throw const ChatboxArchiveException('Unsafe ZIP entry path: (empty)');
    }
    if (path.contains('\\') ||
        path.startsWith('/') ||
        _windowsDrive.hasMatch(path)) {
      throw ChatboxArchiveException('Unsafe ZIP entry path: $path');
    }
  ...
  static List<Map<String, dynamic>> _rewriteContentPart(
    ...
    if (type == 'image') {
      ...
      part['url'] = uri;
      // 资源字节已随包带来并落到本地：显式标注，避免解析器把它当成
      // “只有引用、没有内容”的旧版条目。
      part['chatboxLocalResource'] = true;
      return [part];
    }
    if (type == 'tool-call') {
  ...
  static List<Map<String, dynamic>> _rewritePicture(
    ...
    picture['url'] = uri;
    picture['chatboxLocalResource'] = true;
    return [picture];
  }
  ...
  static void _applyFileStorageKey(
    ...
    file['chatboxLocalResource'] = true;
  }
  ...
  static Map<String, dynamic> _decodeJsonObject(Uint8List bytes, String path) {
    ...
    if (decoded is! Map) {
      throw ChatboxArchiveException('Invalid JSON object entry: $path');
    }
  ...
  static List<dynamic> _decodeJsonArray(Uint8List bytes, String path) {
    final decoded = _decodeJsonValue(bytes, path);
    if (decoded is! List) {
      throw ChatboxArchiveException('Invalid JSON array entry: $path');
    }
    return decoded;
  }
  ...
  static Object? _decodeJsonValue(Uint8List bytes, String path) {
    if (bytes.length > maxJsonEntryBytes) {
      throw ChatboxArchiveException('Backup JSON entry is too large: $path');
    }
    try {
      return jsonDecode(utf8.decode(bytes, allowMalformed: false));
    } catch (_) {
      throw ChatboxArchiveException('Invalid JSON in backup entry: $path');
    }
  }
