...
/// Ensures the app holds direct filesystem access before importing a folder.
///
/// Returns true when access is available (already held or just granted) and
/// false when the user declined or the grant could not be obtained. The folder
/// picker in [FilePicker] only returns a path; it does not grant the raw
/// filesystem access that reading the folder requires.
Future<bool> _ensureDirectoryStorageAccess(
  BuildContext context,
  AppLocalizations l10n,
) async {
  final channel = WorkspaceChannel();
  if (!WorkspaceChannel.isSupportedPlatform) return true;
  if (await channel.hasDirectoryStorageAccess()) return true;
  ...
  final granted = await channel.requestDirectoryStorageAccess();
...
Future<Workspace?> _importFromFolder(BuildContext context, String name) async {
  ...
  // Importing a folder copies real bytes out of it, so the same direct
  // filesystem access that workspaces and the shell need is required here too.
  // The folder picker alone does not grant that: it only hands back a path.
  if (!await _ensureDirectoryStorageAccess(context, l10n)) return null;
  ...
  // The user may have revoked access while the picker was open; verify again
  // so a silent empty copy can never be reported as a success.
  final channel = WorkspaceChannel();
  if (WorkspaceChannel.isSupportedPlatform &&
      !await channel.hasDirectoryStorageAccess()) {
    if (!context.mounted) return null;
    showAppSnackBar(
      context,
      message: l10n.workspaceExternalStorageMessage,
...
Future<void> _copyDirectoryTree(
  ...
  // Without direct filesystem access the listing can come back empty even
  // though the folder is full, which used to be reported as a successful
  // import. Probe readability first so a permission problem fails loudly.
  final probe = Directory(source.path).list(followLinks: false);
  try {
    await probe.isEmpty;
