...
以下能力来自上游基座（当前基座：Kelivo `1.3.0`），JO-AIClient 保持同步。
...
- 🔑 **账号登录** - ChatGPT、Grok、Kimi Code、Claude 等供应商可直接用账号登录，无需手填 API 密钥。
- 🔎 **设置搜索** - 设置页提供搜索入口，输入名称即可跳转到对应设置项并高亮。
- 🕒 **世界书定时生效** - 世界书条目可设置持续条数、冷却和延迟，并支持按会话单独指定提示词与世界书。
- 📱 **手机端 MCP 绑定工作区** - 手机沙盒里的命令行 MCP 服务可绑定到某个工作区，按绑定访问其文件。
