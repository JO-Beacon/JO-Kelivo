...
当前源码版本为 JO-Kelivo `0.1.7+7`，采用原版 Kelivo `1.2.2+66`（tag `v1.2.2`，commit `57ab5404d04dc4c02533d027801d05cdf4460985`）作为实现基座。JO-Kelivo 保持独立 Git 历史；外部基座是可替换的实现输入，不代表重新连接或依赖原版提交祖先。
...
**数据兼容策略**：JO-Kelivo 与同代原版 Kelivo 保持运行时数据隔离，同时保证双方共同支持的数据能够双向导入导出。JO-Kelivo `0.1.7` 继续使用 Kelivo `1.2.2` 的 SQLite/Drift、结构化消息部件和备份契约；JO-Kelivo `0.1.6` 与原版 Kelivo `1.2.1` 的既有合成契约测试仍覆盖会话、消息、版本关系、结构化附件、助手和供应商。JO-Kelivo 还支持从 JO-Kelivo `0.1.5` 原数据目录升级，以及把 JO-Kelivo `0.1.5` 备份导入干净安装。升级前会保留恢复备份；无法解码的损坏记录按当前基座的容错规则跳过并报告。

JO-Kelivo `0.1.7` 不要求向旧 JO-Kelivo `0.1.5` 降级兼容；这不影响 JO-Kelivo 与同代原版之间的双向兼容。互操作范围限于双方共同支持的数据，跨产品迁移前仍应保留双方各自的完整备份。

# JO-Kelivo 详细维护者改版记录（普通用户可跳过）

[查看维护者改版记录](维护者改版记录.md)

后续替换基座、同步外部实现、重构或批量修改前，必须先阅读 [AGENTS.md](AGENTS.md) 和维护者记录，保护 JO-Kelivo 身份、数据隔离、更新发布规则及已确认产品能力。
